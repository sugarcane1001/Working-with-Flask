...
@app.route("/exp")
def index_explicit():
    resp = make_response({"message":"Hello Wokld"})
...
# curl -X GET -i -w '\n' localhost:5000 (run this in new terminal)
# Invoke-WebRequest -Uri "http://localhost:5000" -Method GET -Headers @{"Accept"="application/json"} (this is the alternative to above code, pycharm allows this but not the above one
...
# curl -X GET -i -w '\n' "localhost:5000/name_search?q=Zian"
